...
from v2.utils.geohash_service import geohash_length_for_error, convert_to_point
...
class GeoManager(models.Manager):
    ...
    def get_geocode(self, *options):
        qs = self.filter(location__distance_lt=(tuple(options), 160)).order_by_distance()
        return qs