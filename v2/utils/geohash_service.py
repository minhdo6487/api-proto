...
def get_geoname_by_location(**kwargs):
    method = 'findNearbyJSON'
    domain = 'http://api.geonames.org/'
    user = 'ontana'
    url = '{0}{1}?lat={2}&lng={3}&username={4}'.format(domain,method,kwargs['lat'],kwargs['lng'],user)
    #try:
    with ur.urlopen(url) as f:
        a = f.read().decode('utf-8')
        d = json.loads(a)
        if "geonames" in d.keys():
            return d["geonames"][0]["adminName1"] or None
        else:
            print (d)
            return None
    time.sleep(3)
